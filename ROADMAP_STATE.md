...
> 独立项目：`VectorControl-Portal`（纯静态站 HTML/CSS/JS，无构建系统）
...
gz (Alibaba Cloud,  / Tailscale )
  ...
  └─ Tailscale 组网: gz ↔ prod ↔ prod ↔ 本机

hk (Azure Hong Kong,  / Tailscale )  [2026-06-10 上线]
  ...
  ├─ 角色: 备用公网入口候选
  ├─ 延迟: → prod 0.5ms, → prod 22ms, → gz 82ms
...
| DNS A 记录 |  (prod Azure) |
| 静态文件位置 | prod:/var/www/vectorcontrol-portal/ |
...
| 阿里云限制 | gz 443 端口被拦截（无 ICP 备案），公网流量必须经 prod |
| 部署方式 | SCP 到 prod via Tailscale IP (root@) |
| 主题 | 翡翠绿深色极简 (accent #10b981)，Hero 辐射光晕 + 玻璃拟态面板 |
| 配置归档 | `deploy/prod.vectorcontrol.tech.conf` (prod 专用) |
...
- ✅ prod rp_filter 从 loose (2) 修复为 strict (1)：`/etc/sysctl.d/99-rp-filter.conf`
...
- ✅ prod: fail2ban active, unattended-upgrades, UFW policy DROP, kernel 6.14.0-1017-azure
- ✅ prod: certbot.timer active, SSL 到期 2026-06-10
- ✅ prod: 仅开放 22/80/443 + Tailscale SOCKS (1080 限定 IP)

**关注项（低风险）：**
- ⚠️ prod: 内存压力 (1.0G/1.9G + 297M swap, disk 55%)
- ⚠️ prod: 内存 469M/847M + 69M swap
- ⚠️ prod: Docker HTTPS (443) 上的 VectorControl 部署无独立 Let's Encrypt（cert 可能在容器内）

**端到端验证（2026-06-10）：**
- Portal: 200 ✅ | API Snapshot: 200 ✅ | API Tokens: 200 ✅ | v1/models: 401 ✅ (需认证)
- gpt-5.4 completion (公网 → prod → gz → metapi → OpenAI): "OK" ✅

### 服务重编排计划（2026-06-10）

> 详细方案: `<local>\docs\service-reshuffle-plan.md`

**目标：** gz ↔ prod 服务互换 + Docker 统一管理

**推荐方案 (A+C)：**
1. metapi + CPA → prod（全新 Docker image，保留 hub.db 统计数据和下游 key）
2. OpenClaw → gz
3. prod 入口不变，upstream 从 gz 改为 prod
