...
> 独立项目：`vectorcontrol-gateway-portal`（纯静态站 HTML/CSS/JS，无构建系统）
...
用户 → DNS (vectorcontrol.tech / www.vectorcontrol.tech)
     ...
     → prod (Azure Singapore, )
       ...
       ├─ /playground/ → proxy_pass http://:4001/ (gz via Tailscale)
       ├─ /api/, /v1/ → proxy_pass http://:4001 (gz via Tailscale)
...
| DNS A 记录 |  (prod Azure) |
| 静态文件位置 | prod:/var/www/vectorcontrol-gateway-portal/ |
