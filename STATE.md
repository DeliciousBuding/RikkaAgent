...
| 限制 | 443 被 ISP 拦截（无 ICP 备案），公网流量必须经 prod |
...
| 目标角色 | 中国用户边缘入口（nginx → proxy prod） |
...
用户 → DNS (vectorcontrol.tech / www.vectorcontrol.tech)
     ...
     → prod (Azure Singapore)
       ...
       ├─ /management → proxy_pass prod:8317 (2ms, 迁移后)
       ├─ /fund/ → alias /var/www/vectorcontrol-fund/ (React SPA 投研系统)
       ├─ /playground/ → proxy_pass prod:4001 (2ms, 迁移后)
       ├─ /api/, /v1/ → proxy_pass prod:4001 (2ms, 迁移后)
       └─ SSL: /etc/letsencrypt/live/www.vectorcontrol.tech/ (certbot + nginx)

prod (DigitalOcean Singapore) [迁移后的 API 后端]
...
hk (Azure East Asia / 真正香港) [中国边缘入口]
  └─ nginx → proxy prod (中国用户 ~20ms → HK → 33ms → SGP1 = ~53ms)
...
| DNS A 记录 |  (prod) |
| 静态文件（服务端） | prod:/var/www/vectorcontrol-portal/ |
| 静态文件（本地） | VectorControl-Portal |
...
| 阿里云限制 | gz 443 被拦截（无 ICP），公网流量必须经 prod |
| 部署方式 | SCP → prod via Tailscale (root@) |
...
| 源 ＼ 目标 | gz () | prod (103.253.145.251) | prod () | **hk (104.214.176.143)** |
|-----------|-------------------|----------------------|---------------------|--------------------------|
...
> Azure VM (prod/hk) 公网 ICMP 被阻断；hk→gz 公网 77ms 与 Tailscale 一致

### 4.3 延迟分析

- **SGP 集群**（prod ↔ prod）：2~4ms，均在新加坡
- **HK ↔ SGP**：32~60ms（真正香港到新加坡的物理距离 ~2500km）
- **HK ↔ GZ**：77ms（香港到广州 ~150km，但经公网路由偏高）
- **gz 孤岛**：至 SGP 集群 87~339ms；至本机/doris 10~17ms（均在中国境内）
- **OpenAI 可达性**: gz=✘(GFW), prod/prod=✅(401), hk=✘(403地区封锁)
- **最优 API 路径**：
  - 国际: 用户→prod(0ms)→prod(2ms) = **2ms**
  - 中国: 用户→hk(~20ms)→prod(33ms) = **~53ms**
  - 当前: prod(87ms)→gz(87ms)→SOCKS→prod(87ms)→OpenAI = **350ms+**
- **结论**：metapi+CPA 迁至 prod 后，国际 87ms→2ms(-97%), 中国加 HK 边缘后 350ms→53ms(-85%)
