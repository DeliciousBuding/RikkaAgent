...
- Modular design so apps can reuse the SSH core without adopting the full UI
  - Connection reuse is implemented via `SshConnectionPool`
...
- `:core:ssh` — SSH interfaces: SshExecRunner, ExecEvent, SshExecRunnerFactory, SshConnectionPool
  - `SshjExecRunner` supports optional connection reuse for non-pooled flows
...
2. `SshConnectionPool` ensures an authenticated SSH session exists
...
- `SshExecRunnerFactory` creates per-profile runners wired to the pooled connection.
