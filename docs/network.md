...
通过 `ipinfo.io` 验证（2026-06-10）：旧 "hk" () 实际在新加坡。
...
| 节点 | 位置 | 结果 | 详情 |
|------|------|------|------|
| prod | 新加坡 | ✅ 401 (0.23s) | API 可达，仅认证失败 |
| prod | 新加坡 | ✅ | 同上 |
| gz | 广州(中国) | ✘ 000 (5.0s) | GFW 阻断，curl 超时 |
| **hk** | **香港** | **✘ 403 (0.034s)** | **OpenAI 地区封锁** |

**结论**：
- metapi+CPA 必须部署在**新加坡节点 (prod)**
- **OpenClaw** 是个人 AI 助手（非 API 网关），通过调用 metapi 间接访问模型 → 可部署到 gz（gz→prod RPC 延迟可接受）
...
| **prod** | 338ms | — | 4ms | **33ms** | 78ms |
| **prod** | 87ms | 2ms | — | **60ms** | 69ms |
...
> ✘ = Azure 屏蔽 ICMP（prod/hk 公网 ping 不通，但 TCP 服务正常）
...
| prod | DO | 2GB | 58GB | 1024MB | VectorControl Docker+danted |
...
| 节点 | 位置 | RAM | 角色 |
|------|------|-----|------|
| prod | DO 新加坡 | 2GB | **API 后端** (metapi+CPA+VectorControl) |
| prod | Azure 新加坡 | 847MB | **国际边缘** (nginx SSL → proxy prod) |
| hk | Azure 香港 | 1GiB | **中国边缘** (nginx SSL → proxy prod) |
...
```
prod (2GB):  metapi Docker(62MB) + CPA Docker(54MB) + VectorControl
             OpenClaw 移走后: ~577MB → 加 116MB = ~693MB (35% of 2GB) ← 充裕!
prod (847MB): nginx edge → proxy prod:4001 (2ms)
hk (真HK):   nginx edge + 中国入口 → proxy prod:4001 (33ms)
...
| 中国 API 延迟 | ~370ms (prod→gz→SOCKS回prod) | **~53ms** (→HK→prod) | **-86%** |
| CPA→OpenAI | SOCKS5 代理 | 直连 (prod在新加坡) | 去除代理 |
| gz 内存 | 659MB | ~847MB (加OpenClaw) | +188MB |
| prod 内存 | 1024MB | ~693MB (移走OpenClaw) | **-331MB 释放!** |
...
# 2. 配置 nginx → proxy prod:4001 via Tailscale
...
中国用户完整路径: 用户(~20ms)→HK(nginx:443)→Tailscale(33ms)→prod(metapi) = **~53ms**
...
- OpenClaw → metapi 调用路径: gz → prod (339ms Tailscale / 87ms via prod)

```bash
# gz 上直接用 npm 或 Docker
ssh gz 'npm install -g openclaw@2026.3.8'
# 配置 openclaw.json 的 metapi endpoint 指向 prod
```
...
```
prod/docker-compose.yml:  metapi + cpa + vectorcontrol(已有)
...
| prod 负载 | OpenClaw+VectorControl | metapi+CPA+VectorControl | metapi+CPA+VectorControl |
