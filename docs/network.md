...
>
> **v4 更新**：prod(原假HK)正在迁移至真正的 Azure East Asia（香港）区域。
...
通过 `ipinfo.io` 验证：
- **hk ()**：`city: Singapore, region: Singapore, country: SG`
- **prod ()**：`city: Singapore, region: Singapore, country: SG`

"hk" 并非在香港，而是 Azure 新加坡的另一台机器。以下统一改称 **prod**。
...
用户 → prod(nginx:443)
     → 87ms → gz(metapi:4001)
     → gz(CPA:8317)
     → SOCKS5 → 87ms → prod(danted:1080)
     → OpenAI api.openai.com
     → 响应原路返回：OpenAI → prod → 87ms → gz → 87ms → prod → 用户
```

一次 API 请求（如 gpt-5.4 completion）涉及 **4 次跨国传输 ≈ 350ms** 纯地理开销。

**原因**：gz（广州）无法直接访问 OpenAI（`curl api.openai.com` 超时），CPA 必须通过 prod 的 SOCKS 代理出口。

### 1.3 prod 可以直接访问 OpenAI（但迁移后将不能）

```
$ ssh prod 'curl -s -o /dev/null -w "%{http_code} %{time_total}s" https://api.openai.com/v1/models'
401 0.228947s   ← 目前在新加坡，API 可达
```

**但 prod 正在迁移到真正的 Azure 香港 (East Asia)**。香港不在 OpenAI 支持区域列表中，迁移后将无法访问 OpenAI。

**因此：metapi+CPA 必须部署在新加坡节点（prod）。**
...
> \* 高抖动（prod-ts: 59-352ms, doris-ts: 104-666ms）

### 2.2 公网

| 源 ＼ 目标 | gz | prod | prod | prod |
|-----------|-----|------|------|------|
| **本机** | 22ms | 391ms | ✘ | ✘ |
| **gz** | — | 340ms | ✘ | ✘ |
| **prod** | 340ms | — | ✘ | ✘ |
...
> ✘ = Azure 屏蔽 ICMP（prod/prod 公网 ping 不通，但 TCP 服务正常）
...
│             新加坡集群（<5ms 互联）                │
│                                                   │
│   prod (DO, 2GB)  ←2ms→  prod (Azure, 847MB)    │
│       ↕ 4ms                    ↕ 0.6ms           │
│              prod (Azure, 847MB)                  │
└────────────────────┬────────────────────────────┘
                     │ 87~110ms
...
| prod | DO | 2GB | 58GB | 1024MB | OpenClaw(447MB)+VectorControl Docker+danted |
| prod | Azure | 847MB | — | 469MB | nginx edge(SSL终止) |
| prod/hk | Azure | 847MB | 29GB | ~200MB | **空闲**（正迁移至真HK） |

### 迁移后的将来拓扑

| 节点 | 位置 | RAM | 角色 |
|------|------|-----|------|
| prod | DO 新加坡 | 2GB | **API 后端** (metapi+CPA+OpenClaw+VectorControl) |
| prod | Azure 新加坡 | 847MB | **国际边缘** (nginx SSL → proxy prod) |
| hk | Azure 香港(真) | 847MB | **中国边缘** (nginx SSL → proxy prod) + 监控 |
...
> prod 迁移至真 HK 后，OpenAI 不可达 → metapi+CPA 只能放在 prod 或 prod。

### 方案 A：metapi+CPA → prod（推荐 ✅）

```
prod (2GB):  metapi Docker(62MB) + CPA Docker(54MB) + OpenClaw(447MB) + VectorControl
             预计内存: 1024MB + 116MB = ~1140MB (56% of 2GB)
prod (847MB): nginx edge → proxy prod:4001 (2ms)
hk (真HK):   nginx edge + 中国入口 → proxy prod:4001 (~35ms)
gz (1.6GB):  监控 cron + 备份（释放 metapi+CPA 后 ~400MB）
```

| 指标 | 迁移前 | 迁移后 | 变化 |
|------|--------|--------|------|
| 国际 API 延迟 | 350ms (4×跨国+SOCKS) | **2ms** (prod→prod) | **-99.4%** |
| 中国 API 延迟 | ~370ms (prod→gz→SOCKS回prod) | **~55ms** (→HK→prod) | **-85%** |
| CPA→OpenAI | SOCKS5 代理 | 直连 (prod在新加坡) | 去除代理 |
| gz 内存 | 659MB | ~400MB | -40% |
| prod 内存 | 1024MB | ~1140MB (56%) | +116MB |

**优势**：
- 最大内存节点 (2GB)，能容纳全部 API 服务
- CPA 直连 OpenAI（不需要 SOCKS 代理）
- prod→prod 仅 2ms
- 真 HK→prod ≈35ms（远优于当前 350ms）
- gz 彻底释放 API 负载
- 全部 API 服务 Docker 化，compose 声明式管理

**风险**：
- prod 已有 297MB swap 使用，加 116MB 后需要监控
- 建议给 Docker 容器设 `mem_limit`（metapi 128MB, CPA 96MB）
- 所有 API 在一台机器 = 单点故障（但 Docker 恢复到其他 SGP 节点 <5min）

### 方案 B：metapi+CPA → prod alongside nginx

```
prod (847MB): nginx edge + metapi(62MB) + CPA(54MB) = ~585MB (69%)
```

API 请求在 prod 上本地完成 (0ms)，但 847MB 上同时跑 nginx+metapi+CPA 偏紧，无余量处理流量峰值。**不推荐**。
...
CPA `config.yaml` 需修改：
- **删除** `proxy-url: socks5://:1080`（prod 可直连 OpenAI）
...
```bash
# 在 prod 上准备目录结构
ssh prod 'mkdir -p /opt/services/{metapi-data,cpa}'

# 传输数据
scp /tmp/hub.db.export prod:/opt/services/metapi-data/hub.db
scp /tmp/CLIProxyAPI prod:/opt/services/cpa/
scp /tmp/cpa-config.yaml prod:/opt/services/cpa/config.yaml
scp -r /tmp/cpa-auth/ prod:/opt/services/cpa/auth/
```

```yaml
# prod:/opt/services/docker-compose.yml
services:
  metapi:
...
```bash
# 启动
ssh prod 'cd /opt/services && docker compose up -d'

# 验证本地
ssh prod 'curl -s localhost:4001/v1/models | head -c 200'
ssh prod 'curl -s localhost:8317 | head -c 200'
```

### 5.4 阶段四：切流

```bash
# prod nginx 配置：upstream 从 gz → prod
# /etc/nginx/conf.d/vectorcontrol.tech.conf:
#   proxy_pass http://:4001  →  proxy_pass http://100.117.129.78:4001
#   proxy_pass http://:8317  →  proxy_pass http://100.117.129.78:8317
...
# 3. 配置 nginx → proxy prod:4001 via Tailscale
# 4. 可选：DNS 添加 HK A 记录（或 Cloudflare 地理路由）
```

真 HK 作为中国用户边缘入口：
- 中国→HK: ~20ms → prod: ~35ms → OpenAI 直连
- 比当前 350ms 提升 **85%+**

### 5.7 后续：OpenClaw Docker 化（prod）
...
### 当前
- `vectorcontrol.tech` / `www.vectorcontrol.tech` → A:  (prod)

### 迁移后选项

| 指标 | 仅 prod | prod为主+HK备用 | 双 A 记录 | Cloudflare 地理路由 |
|------|---------|---------------|----------|-------------------|
| DNS 配置 | 不变 | A: prod, 手动切 | A: prod + hk | CNAME → CF |
| SSL 管理 | prod 一台 | 两台 certbot | 两台 certbot | CF 边缘 SSL |
| 中国用户延迟 | ~90ms→prod | ~20ms→HK | 混合 | ~20ms→HK |
| 高可用 | 无 | 手动 | DNS 轮询 | 自动故障转移 |
| 复杂度 | **最低** | 低 | 中 | 高 |

**阶段性推荐**：
1. **立即**：保持 prod 单入口（先完成 metapi 迁移，不增加变量）
...
1. **立即**：CPA Docker 化（配合迁移到 prod）
2. **短期**：OpenClaw Docker 化（prod 上）
3. **保持 host**：nginx (edge proxy 最好直接跑 host), Tailscale (系统级网络), danted

### 每节点 docker-compose 策略

```
prod/docker-compose.yml:  metapi + cpa + openclaw(后续) + vectorcontrol(已有)
prod: host-level nginx only
...
tar czf /backup/prod-$(date +%F).tar.gz \
  /opt/services/metapi-data/ \
  /opt/services/cpa/ \
  /opt/services/docker-compose.yml
# 异地备份到 gz
scp /backup/prod-$(date +%F).tar.gz gz:/backup/
```

**紧急迁移流程**（prod 故障时）：
1. 从 gz 备份恢复到 prod（或真 HK 如果能访问 OpenAI 的备选路径，需要 SOCKS 代理）
2. `docker compose up -d`
3. 修改 prod nginx upstream 为 localhost 或新节点
4. 整个流程 <10 分钟
...
```
         ...
         nginx edge + SSL termination           
           ├── /api, /v1 → prod:4001 (2ms)    
           ├── /management → prod:8317 (2ms)   
           ├── /fund → static files             
           └── / → static portal                
                    │                            
                    │ 2ms (Tailscale)            
...
中国用户 → hk (nginx:443, ~20ms)
         → Tailscale → prod:4001 (~35ms)
         → metapi → CPA → OpenAI (直连)

国际用户 → prod (nginx:443)
         → Tailscale → prod:4001 (2ms)
         → metapi → CPA → OpenAI (直连)
```
