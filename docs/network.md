...
| prod | Azure | 847MB | 29GB | ~200MB | **空闲**（刚初始化） |
...
```
prod (847MB):  metapi Docker(62MB) + CPA Docker(54MB) = ~116MB
prod (2GB):    OpenClaw(447MB) + VectorControl Docker + danted
prod (847MB):  nginx edge → proxy prod:4001(0.6ms)
gz (1.6GB):    monitoring cron + 备份存储（释放metapi+CPA后仅~400MB）
```

| 指标 | 迁移前 | 迁移后 | 变化 |
|------|--------|--------|------|
| API 延迟 (prod→backend) | 87ms + SOCKS回程87ms | 0.6ms 直连 | **-99.3%** |
| CPA→OpenAI | SOCKS proxy | 直连 | 去除代理 |
| gz 内存占用 | 659MB | ~400MB | -40% |
| prod 内存占用 | ~200MB | ~316MB | +116MB（剩余500+MB） |
| prod 变化 | — | — | 无影响 |

**优势**：
- prod 完全空闲，无服务冲突
- CPA 不再需要 SOCKS 代理（新加坡直连 OpenAI）
- prod→prod 仅 0.6ms，几乎零延迟
- gz 变轻，只跑监控脚本和备份
- 三个 SGP 节点 <5ms，未来服务漂移几乎无感

**劣势**：
- prod 仅 847MB，但 116MB 服务占用留有充足余量
- 需要构建 CPA Docker 镜像

### 方案 B：metapi+CPA → prod（集中式）

```
prod (2GB):  metapi+CPA(116MB) + OpenClaw(447MB) + VectorControl = ~600MB+
```

| 指标 | 值 |
|------|-----|
| API 延迟 | prod→prod = 2ms |
| prod 内存 | 1024MB → ~1140MB（swap 风险上升） |

**劣势**：prod 已在内存压力下（1024MB/2GB + 297MB swap），集中更多服务加剧风险。单点故障。

### 方案 C：全服务集中到 prod，VectorControl 迁出到 prod

```
prod (2GB):  metapi+CPA+OpenClaw
prod (847MB): VectorControl Docker
```
...
需修改 `config.yaml`:
- **删除** `proxy-url: socks5://:1080`（新加坡不需要代理）
...
# 4. 传输到 prod
scp /tmp/hub.db.export prod:/root/metapi-data/
scp -r /tmp/cliproxy-backup/ prod:/root/cpa/
```
...
```bash
# prod nginx 配置修改
# 将 upstream 从 gz Tailscale IP 改为 prod Tailscale IP
# /etc/nginx/conf.d/vectorcontrol.tech.conf:
#   proxy_pass http://:4001  →  proxy_pass http://:4001
#   proxy_pass http://:8317  →  proxy_pass http://:8317
...
### 选项：prod vs prod 作为公网入口

| 指标 | 保持prod | 改为prod | 双A记录(prod+prod) |
|------|---------|---------|-------------------|
| 配置变更 | 无 | DNS+SSL迁移 | 两台都要nginx+SSL |
| SSL 管理 | 1台certbot | 1台certbot | 2台certbot |
| 到后端延迟 | 0.6ms→prod | localhost | 混合 |
...
理由：
1. prod→prod 仅 0.6ms，再加一层代理的延迟可忽略
2. prod 作为纯后端不暴露公网，减少攻击面
3. 避免双机 SSL 管理的复杂度
4. prod 已有完善的 nginx + certbot + UFW 配置

如果未来需要高可用：再在 prod 配置 nginx + certbot，通过 Cloudflare/DNS 健康检查实现自动故障转移。
...
```
prod/docker-compose.yml:  metapi + cpa
prod/docker-compose.yml:  openclaw + vectorcontrol (已有)
...
tar czf /backup/prod-$(date +%F).tar.gz metapi-data/ cpa/ docker-compose.yml

# prod 上:
tar czf /backup/prod-$(date +%F).tar.gz openclaw/ vectorcontrol/ docker-compose.yml
```

**紧急迁移流程**（任意 SGP 节点故障时）：
1. 在存活节点上 `docker-compose up -d`（compose 文件 + 数据卷）
2. 修改 prod nginx upstream 指向新节点
...
```
         ...
         nginx edge + SSL termination           
           ├── /api, /v1 → prod:4001 (0.6ms)  
           ├── /management → prod:8317 (0.6ms) 
           ├── /fund → static files             
           └── / → static portal                
                    │                            
                    │ 0.6ms                      
...
将 ssh alias `hk` 改为 `prod`，反映实际地理位置：
